...
namespace VirtoCommerce.CatalogExportImportModule.Data.Services
{
    public sealed class CsvImportErrorReporter : ICsvImportErrorReporter
    {
        ...
        public CsvImportErrorReporter(IBlobStorageProvider blobStorageProvider, string filePath, string header, string delimiter)
        {
            _filePath = filePath;
        ...
        {
            using (await AsyncLock.GetLockByKey(_filePath).GetReleaserAsync())
            {
                    ...
                    .Select(parsingErrorsPerRow =>
                    {
                        ...
                        var errorMessages = parsingErrorsPerRow.Select(parsingErrorsPerCode =>
                        {
                            var errorMessagesPerCode = parsingErrorsPerCode.Select(sameCodeParsingErrors =>
                            {
                                ...
                                var parameters = sameCodeParsingErrors.SelectMany(parsingError => parsingError.Parameters);
