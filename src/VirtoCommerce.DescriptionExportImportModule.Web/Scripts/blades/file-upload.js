...
.controller('virtoCommerce.descriptionExportImportModule.fileUploadController',
    ['FileUploader', '$document', '$scope', '$timeout', 'platformWebApp.bladeNavigationService', 'platformWebApp.assets.api', '$translate', 'platformWebApp.settings', '$q', 'platformWebApp.dialogService', 'virtoCommerce.descriptionExportImportModule.import',
        ...
        blade.title = 'descriptionExportImport.blades.file-upload.title';
        blade.subtitle = 'descriptionExportImport.blades.file-upload.subtitle';
        ...
        blade.dataType = { key: 'Descriptions', value: 'Description' };
        blade.availableDataTypes = [{ key: 'Descriptions', value: 'Description' }];

        function initialize () {
            resetState();

            settings.getValues({ id: 'DescriptionExportImport.Import.FileMaxSize' }, (value) => {
                if (!!value) {
        ...
        $scope.bladeClose = () => {
            if (blade.csvFilePath) {
                bladeNavigationService.showConfirmationIfNeeded(true, true, blade, () => { bladeNavigationService.closeBlade(blade, () => {
                    removeCsv();
                    resetState();
                }); }, () => {}, "descriptionExportImport.dialogs.csv-file-delete.title", "descriptionExportImport.dialogs.csv-file-delete.subtitle");
            } else {
        ...
        $scope.deleteAndCloseBlades = (file) => {
            ...
            dialogService.showDialog(
                dialog,
                "Modules/$(VirtoCommerce.DescriptionExportImport)/Scripts/dialogs/descriptionDeleteAndClose-dialog.tpl.html",
        ...
        $scope.translateErrorCode = (error) => {
            var translateKey = 'descriptionExportImport.validation-errors.' + error.errorCode;
